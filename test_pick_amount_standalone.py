#!/usr/bin/env python3
"""
Standalone verification test for the "low to high" random amount feature.
Tests pick_amount() logic without importing the full db module.
"""
...
print("=" * 80)
print("PICK_AMOUNT FUNCTION VERIFICATION (Standalone)")
print("=" * 80)

# ============================================================================
# TEST 1: Extract and test pick_amount() function standalone
# ============================================================================
print("\n[TEST 1] Testing pick_amount() function logic (extracted)")
print("-" * 80)

# Extract the pick_amount function code
db_path = Path("/app/frontend/LS_Python/agent/db.py")
with open(db_path, 'r') as f:
    db_source = f.read()

# Find and extract the pick_amount function
db_ast = ast.parse(db_source)
pick_amount_node = None
for node in ast.walk(db_ast):
    if isinstance(node, ast.FunctionDef) and node.name == "pick_amount":
        pick_amount_node = node
        break

if not pick_amount_node:
    print("✗ FAIL: pick_amount function not found in db.py")
    exit(1)

print("✓ Found pick_amount function in db.py")

# Extract the function source
func_start = pick_amount_node.lineno - 1
func_end = pick_amount_node.end_lineno
func_lines = db_source.split('\n')[func_start:func_end]
func_source = '\n'.join(func_lines)

# Also need the global variables
globals_code = """
import random
import threading
...
exec(globals_code + '\n' + func_source, globals())
pick_amount = globals()['pick_amount']

print("✓ Successfully extracted and loaded pick_amount function")
...
if enqueue_view:
    ...
    if func_source and '"view_total"' in func_source and 'pick_amount' in func_source:
        ...
        # Verify the bucket format
        if 'f"view:{target_id}"' in func_source or '"view:"' in func_source:
            print("  ✓ Bucket format includes target_id (view:target_id)")
    else:
...
if enqueue_react:
    ...
    if func_source and '"react_total"' in func_source and 'pick_amount' in func_source:
        ...
        # Verify the bucket format
        if 'f"react:{target_id}"' in func_source or '"react:"' in func_source:
            print("  ✓ Bucket format includes target_id (react:target_id)")
    else:
...
worker_path = Path("/app/frontend/LS_Python/agent/worker.py")
...
userbot_path = Path("/app/frontend/LS_Python/agent/userbot.py")
