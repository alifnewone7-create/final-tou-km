...
# ---------------------------------------------------------------------------
# "Low to high" amount picker
# ---------------------------------------------------------------------------
# When a channel is configured with a range (e.g. views 10-20), the exact amount
# for a post is decided ONCE, INSTANTLY, at the moment the job is enqueued (i.e.
# before the view/reaction task starts) and then carried on the job payload. All
# shard copies of that job inherit the same number, so the range stays a single
# global target.
#
# The pick uses the OS entropy source (SystemRandom) instead of a seeded PRNG so
# consecutive posts get genuinely scattered amounts (12, 18, 15, 10, 19, ...).
# We additionally refuse to repeat the previous amount for the same channel back
# to back, which is what makes the sequence *look* random rather than sticky.
_sysrand = random.SystemRandom()
_last_amount: dict[str, int] = {}
_last_amount_lock = threading.Lock()


def pick_amount(low: int, high: int, bucket: str = "") -> int:
    """Pick a scattered random amount inside [low, high].

    Returns 0 when no range is configured (high <= 0), which means "use every
    logged-in userbot" - the original behavior.
    """
    lo = max(0, int(low or 0))
    hi = max(0, int(high or 0))
    if hi <= 0:
        return 0
    if lo > hi:
        lo, hi = hi, lo
    ...
    n = _sysrand.randint(lo, hi)
    with _last_amount_lock:
        prev = _last_amount.get(bucket)
        if prev is not None and n == prev:
            # Re-roll once so the same number never lands twice in a row; if the
            # re-roll ties again, nudge it to the neighbouring value.
            n = _sysrand.randint(lo, hi)
            if n == prev:
                n = prev - 1 if prev > lo else prev + 1
        _last_amount[bucket] = n
    return n
...
def bump_view_sent(target_id: int, views: int) -> None:
    # Disabled on purpose: the total "views sent" counter is no longer tracked
    # in the database (and is not shown in the UI), so nothing is written here.
    return None
            ...
            json.dumps(
                {
                    ...
                    "view_min": view_min,
                    "view_max": view_max,
                    # Exact amount for THIS post, picked instantly right now
                    # (before the task runs) so every shard uses the same number
                    # and consecutive posts get scattered counts.
                    "view_total": pick_amount(view_min, view_max, f"view:{target_id}"),
                    "mode": mode,
                }
            ),
        ),
...
def bump_reaction_sent(target_id: int, reactions: int) -> None:
    # Disabled on purpose: the total "reactions sent" counter is no longer
    # tracked in the database (and is not shown in the UI).
    return None
            ...
            json.dumps(
                {
                    ...
                    # (before the task runs) so every shard uses the same number
                    # and consecutive posts get scattered counts.
                    "react_total": pick_amount(react_min, react_max, f"react:{target_id}"),
                }
            ),
        ),
    )
