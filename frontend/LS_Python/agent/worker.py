...
async def handle_view_post(job: dict) -> dict:
    ...
    # Exact amount for this post, already picked (instantly) when the job was
    # enqueued. 0 = no range configured, so every warm userbot views.
    view_total = int(payload.get("view_total", 0) or 0)
...
async def handle_react_post(job: dict) -> dict:
    ...
    # Exact amount for this post, already picked (instantly) when the job was
    # enqueued. 0 = no range configured, so every warm userbot reacts.
    react_total = int(p.get("react_total", 0) or 0)
    member_ids = await db.arun(db.get_channel_member_ids, chat_id)
    try:
        count = await userbot.react_post_scheduled(
            chat_id, message_id, emojis, window, react_min, react_max, shard_index, SHARD_COUNT,
            member_ids, mode, react_total,
        )
    except userbot.PostDeleted:
...
async def remember_membership(chat_id: int, member_ids: list[int], non_member_ids: list[int]) -> None:
    ...
    try:
        ...
        print(f"[!] membership update failed for chat {chat_id}: {e}", flush=True)
