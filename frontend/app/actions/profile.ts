...
export async function updateProfiles(input: ProfileEditInput) {
  ...
  // The name list is the master list: never use MORE photos than names. If 15
  // images are sent against a 10-name list, only the first 10 are applied (each
  // paired with one name) and the extras are dropped.
  if (useNamePool && photoAssetIds.length > namePool.length) {
    photoAssetIds.length = namePool.length
  }
