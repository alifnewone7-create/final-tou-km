...
-- The aggregate "total views sent" counter is intentionally NOT stored anymore
-- (it is not shown in the UI either), so drop it from older databases.
ALTER TABLE view_targets DROP COLUMN IF EXISTS views_sent;
...
-- The aggregate "reactions sent" counter is intentionally NOT stored anymore
-- (it is not shown in the UI either), so drop it from older databases.
ALTER TABLE reaction_targets DROP COLUMN IF EXISTS reactions_sent;
