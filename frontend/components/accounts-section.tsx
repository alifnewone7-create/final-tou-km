    ...
    <div className="flex flex-col gap-5">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex w-full min-w-0 flex-wrap items-center gap-2 sm:w-auto sm:justify-end">
          ...
          {/* Confirm before probing every account — nothing runs unless you say yes. */}
          <AlertDialog open={confirmCheck} onOpenChange={setConfirmCheck}>
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Check frozen accounts?</AlertDialogTitle>
                <AlertDialogDescription>
                  {`This probes all ${accounts.length} account${accounts.length === 1 ? "" : "s"} to find the frozen ones. It runs in the background and badges update as it goes.`}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel disabled={checking}>Cancel</AlertDialogCancel>
                <AlertDialogAction onClick={handleCheckFrozen} disabled={checking} className="gap-2">
                  {checking ? <Loader2 className="size-4 animate-spin" /> : <ShieldAlert className="size-4" />}
                  {checking ? "Starting…" : "Yes, check frozen"}
                </AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
